...
pub fn xticks_all(bbox: &ProfileBoundingBox, W: f64) -> Vec<f64> {
    ...
    let mut start = snap_floor(bbox.get_min().x, delta);
    start = start.max(0f64);
    let stop = snap_ceil(bbox.get_max().x, delta);
    let mut p = start;
    while p <= stop {
        ret.push(p);
        p = p + delta;
...
fn yticks_all(bbox: &ProfileBoundingBox, H: f64) -> Vec<f64> {
    ...
    start = start.max(0f64);
    let mut stop = snap_ceil(bbox.get_max().y, delta) + 3f64 * delta;
    while stop - start < delta_min {
        start -= delta;
        start = start.max(0f64);
        stop += delta;
    }
